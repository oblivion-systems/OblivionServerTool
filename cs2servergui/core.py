...
class AppCore:
    ...
    def _apply_arena_size(self, csgo_dir: str, mode: str) -> None:
        """Set the K4-Arenas arena size for the deployed Arena mode.

        K4-Arenas picks rounds from its ``round-settings`` list; each round's
        ``TeamSize`` is the arena size.  When the list is non-empty the plugin
        *replaces* its built-in rounds with it (Plugin.cs: RoundSettings.Count > 0
        → ClearRoundTypes + AddRoundType per entry).

        - **1v1** → the plugin's own default is already pure 1v1 (its bundled
          2vs2/3vs3 rounds ship ``EnabledByDefault=false``).  We just delete any
          stale generated config so the default applies — this also undoes a
          previous 2v2 deploy.
        - **2v2** → write a config whose rounds are all ``TeamSize: 2``.  Explicit
          weapon strings are used (no ``PrimaryPreference`` enum) to avoid any
          enum-serialisation mismatch that would make the plugin reject + regen.

        The config path is the standard CounterStrikeSharp location; a missing
        config is regenerated by the plugin on load, so deleting is safe.
        """
        ...
        if mode != "2v2":
            # 1v1 (or any non-2v2 arena): ensure no generated config lingers so
            # the plugin's default pure-1v1 rounds are used.
            if os.path.isfile(cfg_path):
                try:
                    os.remove(cfg_path)
                    self.log("[plugins]   arenas: cleared generated config → default 1v1")
                except Exception as exc:
                    self.log(f"[plugins]   arenas: could not clear config: {exc}")
            return

        # 2v2: force TeamSize 2 across a small weapon-round rotation.
        two_v_two_rounds = [
            {"TranslationName": "k4.rounds.rifle",  "TeamSize": 2,
             "PrimaryWeapon": "weapon_ak47", "SecondaryWeapon": "weapon_deagle",
             "Armor": True, "Helmet": True},
            {"TranslationName": "k4.rounds.awp",    "TeamSize": 2,
             "PrimaryWeapon": "weapon_awp",  "SecondaryWeapon": "weapon_deagle",
             "Armor": True, "Helmet": True},
            {"TranslationName": "k4.rounds.scout",  "TeamSize": 2,
             "PrimaryWeapon": "weapon_ssg08", "SecondaryWeapon": "weapon_deagle",
             "Armor": True, "Helmet": True},
            {"TranslationName": "k4.rounds.pistol", "TeamSize": 2,
             "SecondaryWeapon": "weapon_usp_silencer",
             "Armor": True, "Helmet": False},
            {"TranslationName": "k4.rounds.deagle", "TeamSize": 2,
        ...
        cfg = {
            ...
            "round-settings": two_v_two_rounds,
            ...
            "allowed-weapon-prefs": {
                "rifle": True, "sniper": True, "smg": True, "lmg": True,
                "shotgun": True, "pistol": True,
            },
            "ConfigVersion": 10,
        }
        try:
            ...
            self.log("[plugins]   arenas: wrote 2v2 round config (TeamSize 2)")
        except Exception as exc:
            self.log(f"[plugins]   arenas: could not write 2v2 config: {exc}")
